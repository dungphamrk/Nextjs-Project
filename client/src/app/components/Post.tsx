...
import { faBookmark as faBookmarkRegular,faComment as faCommentRegular, faHeart as faHeartRegular } from '@fortawesome/free-regular-svg-icons'; // Import biểu tượng bookmark rỗng
...
const Post: React.FC<PostProps> = ({ post, isCommentAreaVisible = true, onOpenCommentModal, onPostLike, onPostBookmark, onPostComment }) => {
    ...
    return (
        <div className="flex flex-col  h-full space-y-2 sm:space-y-0 no-scrollbar">
            ...
            <div className="flex rounded-lg space-x-2 justify-between pb-5">
                <div className="flex space-x-2" onClick={() => goToUserProfile(post.userName)}>
                    <div className="story-avatar">
                        <a href="#" className="block bg-white rounded-full relative">
                            <img className="w-8 h-8 rounded-full object-cover p-0.5 bg-black" src={post.profilePictureUrl} />
                        </a>
                    </div>
                    <div className="flex pt-1">
                        <div className="cursor-pointer font-sans text-sm font-semibold text-white self-center">{post.userName}</div>
                        <div className="text-gray-500 w-5 font-sans text-md font-semibold self-center px-2">•</div>
                        <div className="font-sans text-sm font-light text-[#949494] self-center">{post.createdAt}</div>
                    </div>
                </div>
            ...
            <div className="flex justify-between pt-3">
                <div className="flex space-x-4">
                    <span className="cursor-pointer hover:scale-90" onClick={handleLike}>
                        <FontAwesomeIcon icon={hasLiked ? faHeart : faHeartRegular} /> {/* Thay đổi icon like */}
                    </span>
                ...
                <div className="cursor-pointer hover:scale-90" onClick={handleBookmark}>
                    <FontAwesomeIcon icon={hasBookmarked ? faBookmark : faBookmarkRegular} /> {/* Thay đổi icon bookmark */}
                </div>
            </div>

            {/* Likes */}
            <div className="cursor-pointer font-sans text-sm font-semibold text-white self-start">
                {post.likeCount >= 1 ? `${post.likeCount} Likes` : 'Be the first to like this'}
            </div>
            ...
            <div className="font-sans text-sm text-white flex-col">
                ...
                {post.commentCount > 0 && (
                    <p className="text-md text-left hidden sm:block text-gray-400 cursor-pointer" onClick={() => onOpenCommentModal(post)}>
            ...
            {isCommentAreaVisible && (
                <div className="flex border-b border-slate-800 justify-between ">
                    ...
                    {comment.length > 0 && (
                        <span className="font-sans text-md text-sky-500 cursor-pointer hover:text-white h-6" onClick={handleCommentSubmit}>
                            Post
                        </span>
                    )}
