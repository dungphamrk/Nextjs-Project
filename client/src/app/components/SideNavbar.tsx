...
const SideNavbar = () => {
  ...
  const settings = [
    ...
    { name: 'Your activity', icon: faClockRotateLeft, action: () => router.push('/activity') },
  ...
  return (
    <div className={`${isMoreModalOpen ? '' : 'lights-off'} flex flex-col h-screen space-y-4 flex-nowrap sticky top-0 justify-between`}>
          ...
          {menuItems.map((item, index) => (
            <div key={index} onClick={() => setActiveNavBar(item.name)}>
              <a
                ...
                } ${item.name === activeNavBar ? 'bg-slate-1000 animate-pulse' : ''}`}
              >
                {item.icon ? (
                ...
                {!isNavBarCollapsed && <span className="xl:block hidden font-sans text-md font-normal text-white">{item.title}</span>}
              </a>
            </div>
          ))}
        </div>
      ...
      <div onClick={toggleMoreModal} className="cursor-pointer rounded-full flex space-x-4 p-5 sm:hover:bg-slate-1000 sm:hover:delay-100">
        ...
        <span className={`${isNavBarCollapsed ? 'xl:hidden' : 'block'} font-sans text-md font-semibold text-white`}>More</span>
      </div>
        ...
        <div className="bg-black rounded-lg absolute max-w-xs top-2/4 left-1/2 transform translate-y-1/4 -translate-x-1/2">
          <div className="flex flex-col m-1 max-h-full overflow-y-scroll">
            {settings.map((setting, index) => (
              <div key={index} onClick={setting.action} className="flex w-full py-2 px-3 space-x-3 sm:hover:bg-[#343434] cursor-pointer rounded-full">
                <FontAwesomeIcon icon={setting.icon} />
                <span className="font-sans text-sm font-thin text-white">{setting.name}</span>
              </div>
            ))}
          </div>
        </div>
